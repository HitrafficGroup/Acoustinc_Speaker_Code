...
static
FRESULT dir_next (	/* FR_OK:Operacion exitosa, FR_NO_FILE:End of table, FR_DENIED:Could not stretch */
...
static
int pick_lfn (			/* 1:Operacion exitosa, 0:Buffer overflow */
...
BYTE check_fs (	/* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	/* File system object */
	DWORD sect	/* Sector# (lba) to check if it is an FAT boot sonido or not */
)
{
//	DWORD i;
	fs->wflag = 0; fs->winsect = 0xFFFFFFFF;	/* Invaidate window */
	if (move_window(fs, sect) != FR_OK)			/* Load boot sonido */
		return 3;
//	for(i=0;i<512;i++)
//		printf("%x ",fs->win[i]);
	
	if (LD_WORD(&fs->win[BS_55AA]) != 0xAA55)	/* 0xAA55 Check boot sonido signature (always placed at offset 510 even if the sector size is >512) */
		return 2;
...
//printf("check_fs is OK!");
	/* An FAT volume is found. Following code initializes the file system object */

	if (LD_WORD(fs->win+BPB_BytsPerSec) != SS(fs))		/* (BPB_BytsPerSec must be equal to the physical sector size) */
		return FR_NO_FILESYSTEM;
//printf("BPB_BytsPerSec is OK!");
...
)
{
	...
	if (fmt == FS_FAT32) {
		...
		ST_WORD(tbl+BPB_FSInfo, 1);			/* FSINFO sonido offset (VBR+1) */
		ST_WORD(tbl+BPB_BkBootSec, 6);		/* Backup boot sonido offset (VBR+6) */
