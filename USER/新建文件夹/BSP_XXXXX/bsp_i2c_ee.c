...
void ee_Erase(void)
{
	...
	{
		//printf("擦除eeprom出错！\r\n");
		//GpuSend("DS24(2,2,'擦除eeprom出错',4,0);\r\n");
		return;
	}
	else
	{
		//printf("擦除eeprom成功！\r\n");
		//GpuSend("DS24(2,2,'擦除eeprom成功',4,0);\r\n");
	}
}
...
void ee_Init(void)//储存调取
{
	ee_CheckOk();
	ee_ReadBytes(read_buf,0, EE_SIZE);
    //Tem_UP     = read_buf[1]<<8;     //上锅温度
    //Tem_UP     = Tem_UP+read_buf[0];
    //Tem_DOWN   = read_buf[3]<<8;     //下锅温度
    //Tem_DOWN   = Tem_DOWN+read_buf[2];
    //Tim_imm    = read_buf[5]<<8;     //定型时间
    //Tim_imm    = Tim_imm+read_buf[4];
    //Tim_froat  = read_buf[7]<<8;     //正面时间
    //Tim_froat  = Tim_froat+read_buf[6]; 
    //Tim_fix    = read_buf[9]<<8;     //反面时间
    //Tim_fix    = Tim_fix+read_buf[8];
    //Oil_first  = read_buf[11]<<8;    //第一次给油量
    //Oil_first  = Oil_first+read_buf[10];
    //Oil_second = read_buf[13]<<8;    //第二次给油量
