...
static void UartVarInit(void)
{
	usTxBufSize = UART1_TX_BUF_SIZE;	/* 发送缓冲区大小 */
	usRxBufSize = UART1_RX_BUF_SIZE;	/* 接收缓冲区大小 */
	usTxWrite = 0;						/* 发送FIFO写索引 */
	usTxRead = 0;						/* 发送FIFO读索引 */
	usRxWrite = 0;						/* 接收FIFO写索引 */
	usRxRead = 0;						/* 接收FIFO读索引 */
	usRxCount = 0;						/* 接收到的新数据个数 */
	usTxCount = 0;						/* 待发送的数据个数 */
}


/**
  * @brief  初始化串口硬件设备，启用中断
	*         配置步骤：
	*         (1)打开GPIO和USART1的时钟
...
void USART1_Configuration(void)
{
    ...
    USART_ClearFlag(USART1, USART_FLAG_TC);//清除发送完成标志位标志
}
...
uint8_t UartGetChar(uint8_t *_pByte)
{
	...
	{
		*_pByte = pRxBuf[usRxRead];		/* 从串口接收FIFO取1个数据 */

		/* 改写FIFO读索引 */
		DISABLE_INT();
		if (++usRxRead >= usRxBufSize)//缓冲区读到结尾，
		{
			usRxRead = 0;
		}
		usRxCount--;
