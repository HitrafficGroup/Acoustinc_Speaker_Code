...
static const char * FR_Table[]= 
{
	...
	"FR_NOT_READY????????????ß€???",			             /* (3) The physical drive cannot work */
	"FR_NO_FILE???????????",				                 /* (4) Could not find the file */
	"FR_NO_PATH??°§????????",				                 /* (5) Could not find the path */
	"FR_INVALID_NAME????ßπ?????",		                     /* (6) The path name format is invalid */
	"FR_DENIED?????????????????????????????",         /* (7) Access denied due to prohibited access or directory full */
	"FR_EXIST????????????",			                     /* (8) Access denied due to prohibited access */
	"FR_INVALID_OBJECT?????????????????ßπ",		         /* (9) The file/directory object is invalid */
	"FR_WRITE_PROTECTED????????????ß’????",		             /* (10) The physical drive is write protected */
	"FR_INVALID_DRIVE?????????????ßπ",		                 /* (11) The logical drive number is invalid */
	"FR_NOT_ENABLED?????????????",			                 /* (12) The volume has no work area */
	"FR_NO_FILESYSTEM???????ßπ??FAT??",		             /* (13) There is no valid FAT volume */
	...
	"FR_INVALID_PARAMETER????????ßπ"	                     /* (19) Given parameter is invalid */
};
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
*	?? ?? ?: ??
*********************************************************************************************************
* ?¶œ????:https://blog.csdn.net/fxk416532131/article/details/50277835
*/
...
//                CreateNewFile("armfly.bin", "I am WCX, hahaha!", 17);		/* ????????????,ß’?????????? */
//                break;
...
uint8_t ReceiveProcess(uint8_t *rdata, uint8_t reSize)
{
    ...
    if(pack->cardType == 0x32)
    {
        ...
        else if(pack->packType == 0x60)
        {
            ...
            else if(pack->operaType == 0x02)//ß’???? //Iguala RTC + Horarios
            {
...
static void DeleteDirFile(void)
{
	...
	for (i = 0; i < 20; i++)
	{
		sprintf(FileName, "Speed%02d.txt", i);		/* ?ß’1?¶≤??????? */
...
static void WriteFileTest(void)
{
	...
	/* ????? */
	sprintf(TestFileName, "Speed%02d.txt", s_ucTestSn++);		/* ?ß’1?¶≤??????? */
