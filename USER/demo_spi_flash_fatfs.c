...
{
	"FR_OK?????",				                             /* (0) Operacion exitosa */
	"FR_DISK_ERR????????????",			                 /* (1) Ocurrio un error grave en la capa baja de entrada y salida */
	"FR_INT_ERR?????????",				                     /* (2) Fallo una asercion */
	"FR_NO_FILE???????????",				                 /* (4) No se encontro el archivo */
	"FR_DENIED?????????????????????????????",         /* (7) Acceso denegado due to prohibited access or directory full */
	"FR_EXIST????????????",			                     /* (8) Acceso denegado due to prohibited access */
	...
	"FR_LOCKED?????????????????????????",				 /* (16) La operacion fue rechazada por la politica de uso compartido */
	"FR_NOT_ENOUGH_CORE????????????????????",		     /* (17) no se pudo reservar el buffer de trabajo LFN */
