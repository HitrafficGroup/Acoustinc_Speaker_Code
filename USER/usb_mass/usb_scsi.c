...
void SCSI_Inquiry_Cmd(uint8_t lun)
{
  ...
  //scsi_printf("SCSI_Inquiry_Cmd(%d)\r\n",lun);  
...
{
  scsi_printf("SCSI_ReadFormatCapacity_Cmd(%d)\r\n",lun); 
  ...
  if (MAL_GetStatus(lun) != 0 )
  {
	...
	scsi_printf("SCSI_ReadFormatCapacity_Cmd(%d) Abort\r\n",lun);
    return;
  }
...
{
  scsi_printf("SCSI_ReadCapacity10_Cmd(%d)\r\n",lun);  
  ...
  if (MAL_GetStatus(lun))
  {
    ...
    Set_CSW (CSW_CMD_FAILED, SEND_CSW_ENABLE);
    Bot_Abort(DIR_IN);
    scsi_printf("SCSI_ReadCapacity10_Cmd() Abort\r\n");	
    return;
  }
...
{
  scsi_printf("SCSI_ModeSense6_Cmd(%d)\r\n",lun);  
...
{
  scsi_printf("SCSI_ModeSense10_Cmd(%d)\r\n",lun);  
...
void SCSI_RequestSense_Cmd (uint8_t lun)
{
  ...
  scsi_printf("SCSI_RequestSense_Cmd(%d)\r\n",lun); 
...
{
  scsi_printf("Set_Scsi_Sense_Data\r\n"); 
...
{
  scsi_printf("SCSI_Start_Stop_Unit_Cmd(%d)\r\n",lun);
...
void SCSI_Read10_Cmd(uint8_t lun , uint32_t LBA , uint32_t BlockNbr)
{
  ...
  if (Bot_State == BOT_IDLE)
  {
    ...
    else
    {
      ...
      Set_CSW (CSW_CMD_FAILED, SEND_CSW_ENABLE);
	  scsi_printf("SCSI_Read10_Cmd(), INVALID_FIELED_IN_COMMAND\r\n");
    }
    return;
  }
...
void SCSI_Write10_Cmd(uint8_t lun , uint32_t LBA , uint32_t BlockNbr)
{
  ...
  if (Bot_State == BOT_IDLE)
  {
    ...
    else
    {
	  ...
	  scsi_printf("SCSI_Write10_Cmd(), INVALID_FIELED_IN_COMMAND\r\n");
    }
    return;
  }
...
{
  scsi_printf("SCSI_Verify10_Cmd(%d)\r\n",lun);	
  ...
  else
  {
    ...
    Set_Scsi_Sense_Data(CBW.bLUN, ILLEGAL_REQUEST, INVALID_FIELED_IN_COMMAND);
    Set_CSW (CSW_CMD_FAILED, SEND_CSW_DISABLE);
	scsi_printf("SCSI_Verify10_Cmd(), ILLEGAL_REQUEST\r\n");
  }
}
/*******************************************************************************
...
{
  scsi_printf("SCSI_Valid_Cmd(%d)\r\n",lun);		
  if (CBW.dDataLength != 0)
  {
    Bot_Abort(BOTH_DIR);
    Set_Scsi_Sense_Data(CBW.bLUN, ILLEGAL_REQUEST, INVALID_COMMAND);
    Set_CSW (CSW_CMD_FAILED, SEND_CSW_DISABLE);
	scsi_printf("SCSI_Valid_Cmd(), ILLEGAL_REQUEST\r\n");
  }
  else
    Set_CSW (CSW_CMD_PASSED, SEND_CSW_ENABLE);
}
...
{
  scsi_printf("SCSI_TestUnitReady_Cmd(%d)\r\n",lun);	
  if (MAL_GetStatus(lun))
  {
    Set_Scsi_Sense_Data(CBW.bLUN, NOT_READY, MEDIUM_NOT_PRESENT);
    Set_CSW (CSW_CMD_FAILED, SEND_CSW_ENABLE);
    Bot_Abort(DIR_IN);
	scsi_printf("SCSI_TestUnitReady_Cmd(%d), NOT_READY\r\n",lun);
    return;
  }
  else
  {
...
{
  scsi_printf("SCSI_Invalid_Cmd(%d)\r\n",lun);	
  if (CBW.dDataLength == 0)
  {
    Bot_Abort(DIR_IN);
  }
...
bool SCSI_Address_Management(uint8_t lun , uint8_t Cmd , uint32_t LBA , uint32_t BlockNbr)
{
  if ((LBA + BlockNbr) > Mass_Block_Count )
  {
    ...
    Set_CSW (CSW_CMD_FAILED, SEND_CSW_DISABLE);
	scsi_printf("SCSI_Address_Management(%d), ADDRESS_OUT_OF_RANGE\r\n",lun);
    return (FALSE);
  }
  ...
  if (CBW.dDataLength != BlockNbr * Mass_Block_Size)
  {
    ...
    Set_Scsi_Sense_Data(CBW.bLUN, ILLEGAL_REQUEST, INVALID_FIELED_IN_COMMAND);
    Set_CSW (CSW_CMD_FAILED, SEND_CSW_DISABLE);
	scsi_printf("SCSI_Address_Management(%d), INVALID_FIELED_IN_COMMAND\r\n",lun);
    return (FALSE);
  }
  return (TRUE);
}
